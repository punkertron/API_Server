...
crow::response filesUpload(const crow::request &req)
{
    ...
    try
    {
        ...
        c.prepare("check", "SELECT file_id FROM server.files_info WHERE name = $1 AND user_id = $2");

        c.prepare("find_file_id", "SELECT file_id FROM server.files_info WHERE name = $1 AND user_id = $2");
...
crow::response filesDelete(const crow::request &req)
{
    try
    {
        ...
        if (!r[0].as<bool>())
        {
            c.prepare("update_file_status",
                      "UPDATE server.files_info SET is_deleted = true WHERE "
                      "file_id = $1");
